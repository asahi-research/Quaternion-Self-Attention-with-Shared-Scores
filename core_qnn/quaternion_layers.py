...
def moving_average_update(statistic: torch.Tensor, curr_value: torch.Tensor, momentum: float) -> torch.Tensor:
...
class QuaternionBatchNorm2d(Module):
    ...
    def __init__(self, num_features: int, gamma_init: float = 1., beta_param: bool = True, momentum: float = 0.1) -> None:
...
class QSpectralNorm:
    ...
    def _solve_v_and_rescale(self, weight_mat: torch.Tensor, u: torch.Tensor, target_sigma: torch.Tensor) -> torch.Tensor:
